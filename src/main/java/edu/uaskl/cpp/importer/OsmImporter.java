...
public class OsmImporter {
    ...
    protected static int getDistance(final WayNodeOSM a, final WayNodeOSM b) { // TODO is int the right return type?
        ...
        final double latitudeCos = Math.cos(a.getLatitude()) * Math.cos(b.getLatitude());
        final double longitudeDiff = b.getLongitude() - a.getLongitude(); // is b always > than a? what happens if not?
        final double latitudeValue = latitudeCos * Math.cos(longitudeDiff);

        final double latitudeSin = (Math.sin(a.getLatitude()) * Math.sin(b.getLatitude()));

        return (int) (Math.acos(latitudeSin + latitudeValue) * 6367500); // TODO could Math.round be the better way?
    }
                ...
                if (cNode.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    if (childNode.getNodeName() == "nd") {
                        ...
                        if (childNode.hasAttribute("k") && childNode.getAttribute("k").equals("junction") && childNode.getAttribute("v").equals("roundabout"))
                            roundabout = true;
                        else if (childNode.hasAttribute("k") && childNode.getAttribute("k").equals("name"))
